...
def build_synthesis_prompt(question: str, records: List[Dict[str, Any]], sufficiency: Dict[str, Any], classified_total: int, pending_count: int) -> str:
    ...
    lines.append("2. Evidence-backed themes")
    lines.append("3. Evidence count for each theme")
    lines.append("4. Supporting excerpts and source links")
    lines.append("5. Interpretation clearly labelled as interpretation")
    lines.append("6. Contradictory or minority evidence")
    lines.append("7. Evidence-strength rating")
    lines.append("8. Product implication")
    lines.append("9. Data limitations")

    lines.append("\nRules:")
    lines.append("- Use only the supplied evidence records.")
    lines.append("- Never claim behavior without specific evidence IDs.")
    lines.append("- Do not convert operational complaints into discovery barriers without explicit causal mention.")
    lines.append("- Do not report percentages from fewer than 20 eligible records.")
    lines.append("- When reporting counts, always include denominator.")
    lines.append("- State result is provisional when pending classification count is non-zero.")
    lines.append("- Separate Observation, Interpretation, and Implication labels.")
