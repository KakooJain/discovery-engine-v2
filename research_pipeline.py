...
def build_synthesis_prompt(question: str, records: List[Dict[str, Any]], sufficiency: Dict[str, Any], classified_total: int, pending_count: int) -> str:
    ...
    lines.append('      "eligible_record_count": 0,')
    lines.append('      "supporting_record_ids": [],')
    lines.append('      "evidence_excerpts": [],')
    lines.append('      "product_opportunity": "",')
    lines.append('      "confidence": "low|medium|high"')
    lines.append('    }')
    lines.append('  ],')
    lines.append('  "contradictory_evidence": {')
    lines.append('    "present": false,')
    lines.append('    "summary": "",')
    lines.append('    "record_count": 0')
    lines.append('  },')
    lines.append('  "evidence_status": "sufficient|limited|insufficient",')
    lines.append('  "data_limitations": ""')
    lines.append("}")

    lines.append("\nRules:")
    lines.append("- Use only the supplied evidence records.")
    lines.append("- Never claim behavior without specific evidence IDs from the records above.")
    lines.append("- Return 2 or 3 insights for normal successful answers; if only one insight is defensible, return one insight and mark evidence_status as limited.")
    lines.append("- Each insight must be meaningfully distinct and answer the specific research question.")
    lines.append("- Each insight must include observation, why_it_matters, evidence_count, product_opportunity, and confidence.")
    lines.append("- Use specific behavioural claims, not generic topic labels.")
    lines.append("- Direct answer should be 1-2 sentences and directly answer the question.")
    lines.append("- If contradictory evidence is absent, set present to false and use the exact summary 'No meaningful contradictory evidence found in the retrieved sample.'")
    lines.append("- If contradiction exists, set present to true and summarize it briefly with the record count.")
    lines.append("- Do not include markdown tokens such as ** or ## in visible fields.")
    lines.append("- Do not mention pending classification count in insight text, confidence, or limitations beyond the data_limitations field.")
    lines.append("- Prefer crisp product opportunities over generic product information or quality language.")
